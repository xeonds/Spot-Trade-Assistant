...
<script setup lang="ts" name="af-table-column">
import {
  getCurrentInstance,
  ref,
  reactive,
...
const isFit = computed(() => {
  return (
    (proxy.$parent.$attrs.autoFit === undefined && attrs.fit === undefined) ||
    (proxy.$parent.$attrs.autoFit === false && attrs.fit !== undefined)
  )
})
  ...
  return (
    ...
    nextTick(() => {
      ...
      setTimeout(() => {
        ...
        const bodyWrapper = attrs.fixed
          ? (document as any)
              .querySelector(
                `.el-table__fixed${
                  attrs.fixed === 'right' ? `-${attrs.fixed}` : ''
                }`
              )
        ...
        const nodes = bodyWrapper.querySelectorAll(
          `.${attrs.prop || `encode-${transChar(attrs.label)}`}-column`
        )
        if (nodes.length) {
...
const getMaxLength = (arr: any) => {
  return arr.reduce((length: any, item: any) => {
    if (item) {
      ...
      let newLength =
        charLength * $consts.fontRate.CHAR_RATE +
        numLength * $consts.fontRate.NUM_RATE +
        otherLength * $consts.fontRate.OTHER_RATE
